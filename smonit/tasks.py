...
def respond_minion():
    ...
    if path.isfile(db_path):
        ...
        data_count_not_response = schema.default(
            "total_not_response", count_minions_not_response
        )
    ...
    if respond:
        ...
        if len(result) > 0:
            if "disabled" not in result:
                ...
                data_number_states = schema.with_tag(
                    "number_states", minion, len(states_list)
                )
                ...
                for state in states_list:
                    ...
                    data_number_states_changes = schema.state_changes(
                        minion, state, changes_number
                    )
                    influxdb.write_points(data_number_states_changes)
                    # Add failure changes
                    failure_number = failure_state_changes.count(state)
                    data_number_failure_changes = schema.failure_changes(
                        minion, state, failure_number
                    )
            ...
            data_highstate_issue = schema.with_tag(
                "highstate_issue", minion, highstate_issue
            )
            influxdb.write_multiple_data(data_highstate_issue)

            # Show the total number of failure changes
            data_total_failure_number = schema.with_tag(
                "total_failure_changes", minion, len(failure_state_changes)
            )
            influxdb.write_multiple_data(data_total_failure_number)

            # Show the total number of changes
            data_total_changes = schema.with_tag(
                "total_changes", minion, len(states_changes_list)
            )
            influxdb.write_multiple_data(data_total_changes)

            # Show if highstate is disabled
            data_highstate_disabled = schema.with_tag(
                "highstate_disabled", minion, highstate_value
            )
