        ...
        if values:
            if "disabled".capitalize() not in values:
                for info in values:
                    ...
                    if len(changes) >= 1:
                        # FIXME: Add a sleep time if salt-minion is not initialized. Error: KeyError: 'stdout
                        if changes["stdout"] != "" or changes["stderr"] != "":
                            # Add duration time in changes list
                            changes.update({"time": info["duration"]})

                            changes_list.update(
                                {info["__id__"]: {info["__sls__"]: changes}}
                            )
            else:
                changes_list.update({"disabled": "highstate"})
...
class InfluxDb(object):
    def __init__(self):
        self.host, self.port = os.environ.get(
            "INFLUXDB_ENDPOINT", "localhost:8086"
        ).split(":")
    ...
    def _ping(self):
        ...
        try:
            ...
            log.critical(
                f"It seems influxdb server is down or no any connection to the server"
            )
        return status
